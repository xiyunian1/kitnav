...
function buildSystemPrompt(options: RunnerOptions) {
  return [
    ...
    "- 如果项目内 templates/ 下存在模板，必须按 PPT Master Step 3 读取模板 design_spec.md，并把品牌、版式或整套模板约束写入项目 design_spec.md / spec_lock.md。",
    "- 如需图片或公式，必须先写入 images/image_prompts.json、images/formula_manifest.json 或图片搜索计划，再用 run_ppt_script 执行 image_gen.py、image_search.py、latex_render.py、analyze_images.py；所有引用文件必须位于当前项目目录内。",
    "- 如需动画，先确保顶层 <g> 有稳定 id，可用 animation_config.py scaffold/list-groups/validate 生成或检查 animations.json。",
    "- 如需旁白音频，先完成 notes/total.md 和 total_md_split.py，再用 notes_to_audio.py 生成 per-slide 音频。",
...
function buildInitialTaskPrompt(params: GenerationParams, options: RunnerOptions) {
  ...
  return [
    ...
    "- run_ppt_script：运行白名单 PPT Python 脚本（质量检查、图片/公式处理、动画配置、讲稿音频、SVG 后处理和 PPTX 导出）",
    "",
    "请按以下阶段连续执行：",
