...
export async function POST(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!ACTIVE_STATUSES.includes(project.status as (typeof ACTIVE_STATUSES)[number])) {
    return Response.json({ ok: true, status: project.status, message: "项目不在生成中" });
  }

  const aborted = cancelPptGeneration(project.id);
  const line = `[${new Date().toISOString()}] 用户停止生成`;
  await prisma.pptProject.update({
    ...
    data: {
      ...
      currentPhase: "已停止生成",
      error: "用户已停止生成",
      logs: [project.logs, line].filter(Boolean).join("\n"),
    },
  });
  const refunded = await refundPptProjectCredits(project.id, `PPT 生成停止退款（${project.id}）`);
