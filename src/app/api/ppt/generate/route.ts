...
const requestSchema = z
	.object({
		...
		sourceFileNames: z.array(z.string().trim().max(255)).max(10).optional(),
		templateFileUrls: z.array(z.string().trim().max(2000)).max(1).optional(),
		templateFileNames: z.array(z.string().trim().max(255)).max(1).optional(),
...
export async function POST(req: NextRequest) {
	...
	const storedParams = JSON.stringify({
		...
		sourceFileNames: parsed.sourceFileNames?.slice(0, resolvedFileUrls.length),
		templateFileUrls: uploadedTemplateFileUrls,
		templateFileNames: parsed.templateFileNames?.slice(
			0,
			uploadedTemplateFileUrls.length,
		),
