...
const maxAgeMs = Number(process.env.IMAGE_WORKER_HEALTH_MAX_AGE_MS || 45_000);

function fail(message) {
  console.error(`Image worker unhealthy: ${message}`);
  process.exit(1);
}
...
if (
  ...
  fail("IMAGE_WORKER_HEALTH_MAX_AGE_MS must be an integer from 5000 to 2147483647");
}
...
if (health.started !== true) fail("worker is stopping");
if (
